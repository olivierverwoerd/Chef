"""
Here's where the chef does it's magic.
A few design coises. When said on top. we take this as in front of the stack. AKA 0 as index
This is to make a better visualisation of the result
"""
import random
...
class Kitchen:
    ...
    Here's were are all the modifications done and lists are kept.
    """
    def __init__(self, ingredients, mixingbowls, bakingdishes):
        self.mixingbowls = mixingbowls
        self.bakingdishes = bakingdishes
    ...
    def __str__(self):
        ...
        for item in self.ingredients:
            s += item.__str__() + '\n'
        s += "\nMixingbowls:\n"
        for mixingbowl in self.mixingbowls:
            for item in mixingbowl:
                s += item.__str__() + '\n'
        s += "\nBakingdishes:\n"
        for bakingdish in self.bakingdishes:
    ...
    def get_ingredients(self):
        """
        just a simple getter for more infomation.
        There are no real setters other than a new kitchen
        :return: list of ingredients
        """
        return self.ingredients

    def get_mixingbowls(self):
        """
        just a simple getter for more infomation.
        There are no real setters other than a new kitchen
        :return: list of mixingbowls
        """
        return self.mixingbowls

    def get_bakingdishes(self):
        """
        just a simple getter for more infomation.
        There are no real setters other than a new kitchen
        :return: list of bakingdishes
        """
        return self.bakingdishes
    ...
    def reduce(self, side: str, name: str, operator: str, mixingbowl: int, index=None, last_value=None):
        ...
        if index > 0:
            ...
            self.mixingbowls[mixingbowl] = [Ingredient(int(new_value), self.mixingbowls[mixingbowl][0].get_state(), self.mixingbowls[mixingbowl][0].get_name())]
    ...
    def zipwith(self, bowl_a: int, bowl_b: int, operator='+', index=None):
        ...
        if index == None:
            ...
            if operator == '+':
                self.mixingbowls[bowl_a][index] = Ingredient(int((self.mixingbowls[bowl_a][index].get_amount() + self.mixingbowls[bowl_b][index].get_amount())),
                                                             self.mixingbowls[bowl_a][index].get_state(), self.mixingbowls[bowl_a][index].get_name())
            elif operator == '-':
                self.mixingbowls[bowl_a][index].set_amount(self.mixingbowls[bowl_a][index].get_amount() - self.mixingbowls[bowl_b][index].get_amount())
            elif operator == '*':
                self.mixingbowls[bowl_a][index].set_amount(self.mixingbowls[bowl_a][index].get_amount() * self.mixingbowls[bowl_b][index].get_amount())
            elif operator == '/':
                self.mixingbowls[bowl_a][index].set_amount(self.mixingbowls[bowl_a][index].get_amount() / self.mixingbowls[bowl_b][index].get_amount())
